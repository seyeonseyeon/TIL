...
현재 상영중인 영화의 목록을 보여주고 영화와 날짜 시간을 선택하면 해당 날짜의 상영시간이 표시되도록 하였습니다.

상영시간을 선택하면 다음페이지 이동 버튼이 표시되도록 하여 상영시간을 선택하지 않으면 진행되지 않도록 하였습니다.

또 로그인이 되어있지 않다면 로그인 팝업이 나타나도록 구현 하였고, 해당날짜에 해당 영화의 스케줄이 없다면 그에 대한 안내 텍스트를 구성하였습니다.
...
영화 포스터는 슬라이더 플러그인  Swiper를 사용, 날짜는 JQuery의 위젯인 Datepicker를 사용하여 구현하였습니다.

상영시간의 정보는 사용자의 클릭에 따라 바로 확인 할 수 있도록 Ajax를 사용하여 해당 정보를 불러왔습니다.

상영시간에 대한 정보를 불러오기 위해선 본 프로젝트의 데이터베이스 구조상 2번의 Ajax 통신이 필요했습니다.

때문에 Ajax를 동기 처리 방식으로 설정하여 해당 기능을 구현하였습니다.

선택한 영화의 상영일에 상영스케줄이 없는 경우 안내가 표시되고 상영시간을 선택했을 때 다음 페이지로 진행하는 버튼이 표시되도록 하였습니다.
...
다음 페이지 버튼을 눌렀을 때 로그인 세션이 없다면 로그인 팝업이 나타나며 로그인 페이지로 안내합니다.
...
선택한 스케줄의 좌석정보를 표시합니다. 

페이지가 시작할 때 예매가 완료된 좌석은 회색으로 표시되어 선택이 불가능하도록 구현했습니다.

웹소켓을 이용하여 사용자가 해당페이지에서 있을 때 좌석이 예매 된다면 실시간으로 좌석이 회색으로 칠해지도록 구현했습니다.

좌석을 선택하지 않고 다음페이지 버튼을 눌렀을 때 알람창이 표시되도록 구현하였습니다.
...
좌석 선택 페이지는 페이지가 준비될 때 예매된 좌석에 대한 정보를 불러와 좌석을 표시합니다.

좌석을 선택하지 않고 다음 페이지 버튼을 누르면 알람창이 나타나고 진행되지 않습니다.

좌석 칸을 클릭하여 좌석을 선택하며 좌측에 선택한 좌석 정보와 가격이 나타납니다.
...
타인이 좌석을 선택하면 실시간으로 최신 예매된 좌석 정보를 불러와 표시합니다.

웹소켓으로 좌석 정보를 불러오는 시점은 타인이 결제페이지에서 결제하기를 누르고 결제가 성공하는 시점입니다.
...
사용자가 선택한 스케줄과 좌석의 정보가 출력됩니다.

사용자가 보유한 쿠폰이 있다면 셀렉트 창에 표시되며 보유한 적립금도 표시됩니다.

쿠폰을 선택하고 사용할 적립금을 입력하면 최종금액에서 할인금액을 차감합니다.

만약 예매하고자 하는 좌석이 예매도중 선점된다면 결제가 실패하고 좌석선택 페이지로 이동하게 됩니다.
...
쿠폰을 사용하면 그 금액만큼 최종 금액에서 차감됩니다.

적립금은 보유 적립금이 표시되며 입력한 값만큼 최종금액에서 차감합니다.

입력할 수 있는 적립금은 최종금액을 넘을 수 없습니다. 이때의 최종금액은 쿠폰금액을 제한 최종금액입니다. 

또 보유 적립금보다 많은 값을 입력할 수 없습니다.

이 두가지의 경우보다 높은 값을 입력할 경우 입력할 수 있는 가장 높은 값으로 자동 입력 됩니다.

예1) 최종금액이 16000원이고 보유 적립금이 7000원인 경우 최대 입력값은 7000원

예2)최종 금액이 16000원이고 보유 적립금이 20000원인 경우 최대 입력값은 16000원

결제 시스템은 아임포트(import) 테스트로 카드 결제를 구현하였습니다.
...
예매 진행 도중 좌석이 선점되면 결제가 되지 않고 좌석선택으로 되돌아갑니다.

좌석선점은 데이터베이스에 '예매된 좌석' 테이블을 만들어 스케줄번호,상영회차번호,좌석번호 3가지의 정보를 기본키로 설정하여 구현하였습니다.

특정 스케줄의 특정 상영회차의 좌석번호는 기본키로 중복이 불가하여 다시 입력된 경우 입력이 실패는 방법입니다.
...
만약 사용자가 A1,A2,A3,A4 좌석의 예매를 진행하는 도중 A4좌석이 선점되었을 경우 A1,A2,A3의 좌석이 예매되어 데이터베이스에 반영되고 A4좌석이 실패하고 롤백되는 문제점이 있었습니다.

 이는 다수의 좌석을 예매하는 방식이 반복문으로 데이터베이스에 INSERT되는 방식이기 때문이었습니다.

문제를 해결하기 위해서 해당 반복문을 트랜잭션 어노테이션을 사용해 하나의 트랜잭션 단위로 설정하였습니다.

이 덕분에 만약 반복문이 진행되는 도중 실패하게 된다면 해당 반복문 전체가 롤백되도록 하였습니다.
...
결제 완료시 결제정보가 출력되고 버튼을 클릭시 티켓이 표시됩니다.
